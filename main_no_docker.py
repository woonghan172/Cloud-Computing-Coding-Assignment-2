...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Should input the index of the node, which is one of 1,2, and 3"
    )
    parser.add_argument("--idx", type=int, default=-1,
                        help="node's index starting from 1")

    args = parser.parse_args()

    if args.idx == -1:
        print("Error: --idx must be provided")
        sys.exit(1)

    node_idx = args.idx

    NODES = load_nodes_from_file(NODES_FILE)
    SELF_NODE = NODES[node_idx-1]
    parsed = urlparse(SELF_NODE)
